...
export default function BenefitSection({ data, withPaddingTop = false }) {
    ...
    return (
        <section className={`p-indent-sm ${withPaddingTop && 'lg:pt-indent'} pt-5 lg:px-indent-lg w-screen text-left lg:text-center`}>
            <h2 className="text-title text-red-main my-4 w-1/2 lg:w-full">{textField.title}</h2>
            <SanityBlockContent blocks={textField.content} />
            <aside className='w-full flex flex-wrap items-center justify-between'>
                {benefitImages && benefitImages.map(image => <BenefitImage image={image} key={image._key} />)}
            </aside>
        </section>
    )
}
