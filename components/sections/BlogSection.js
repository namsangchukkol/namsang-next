...
export default function BlogSection({ data }) {
    ...
    return (
        <main className={`w-full flex flex-col px-indent-xsm py-12 ${style.maincontainer}`}>
            <FeaturedImage textField={textFieldBtn} image={singleImage} />
            <section className='grid place-items-center'>
                {blogSection && blogSection.map((list, index) =>
                    <Blog content={list} key={index} />
                )}
            </section>
        </main>
    )
...
const FeaturedImage = ({ textField, image }) => {
        ...
        <section className='w-full justify-start items-center mr-5 '>
            <aside className='relative rounded-lg'>
                <aside className='lg:grid hidden lg:w-2/3 md:w-full rounded-lg overflow-hidden'>
                    <Image {...imgObj} width={400} height={400} objectFit="cover" alt={alt} title={singleImageTitle} />
                </aside>
...
const Blog = ({ content }) => {
    ...
    return (
        <section className={`flex md:justify-around justify-between w-full ${style.blogcontainer}`}>
            <aside className='relative rounded-lg overflow-hidden'>
                {imgObj && <Image {...imgObj} width={200} height={120} objectFit="cover" alt={singleImage?.alt} title={singleImage?.title} className="rounded-lg" />}
            </aside>
            <aside className='pl-6 flex flex-col justify-between'>
                <BlockContent blocks={textField?.content} />
                <Link href={"/"} passHref={true}>
                    <p className="text-sm text-right cursor-pointer hidden lg:grid" href="/">
                        Read More &gt;
                    </p>
                </Link>
            </aside>
