...
export default function Navbar(context) {
  ...
  const { header } = data
  const languageOptions = router.locales.map(locale => <Link href={path} locale={locale} key={locale} passHref><span classNameNameName="mx-5 cursor-pointer">{locale}</span></Link>)
