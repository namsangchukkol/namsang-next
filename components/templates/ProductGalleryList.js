...
export default function ProductGalleryList({ title, more = true, moreButton }) {
    ...
    return (
        <>
            ...
            {more &&
                <Link href="product">
                    <p className='text-center cursor-pointer hover:underline'>{moreButton} {'>'}</p>
                </Link>}
        </>
    )
}

const ImageCard = ({ image, slug, name }) => {
        ...
        <Link href={`/products/${slug}`} passHref>
            <div className='flex flex-col justify-center items-center w-full lg:w-[300px] my-10 cursor-pointer hover:scale-[1.02] transition-all'>
                <div className='relative w-[80vw] h-[300px] lg:w-[350px] lg:h-[300px] rounded-lg overflow-hidden'>
                    ...
                    <div className='absolute top-0 left-0 w-full h-full z-50 hover:bg-red-main opacity-[0.8]
